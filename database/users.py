    ...
    with get_db_connection() as connection:
        with connection.cursor(dictionary=True) as cursor:
            ...
            cursor.execute("""INSERT
                                INTO users
                                     (name, email, region, hashed_password)
                              VALUES (%s, %s, %s, %s)""", [name, email, region, hashed_password])
            connection.commit()
...
def get_user_with_credentials(email, password):
    ...
    with get_db_connection() as connection:
        with connection.cursor(dictionary=True) as cursor:
            password_bytes = password.encode()
            cursor.execute("""SELECT u.id, u.hashed_password
                                FROM users AS u
                               WHERE u.email = %s""", [email])
            matching_user = cursor.fetchone()
...
def get_user_by_id(user_id):
    ...
    with get_db_connection() as connection:
        with connection.cursor(dictionary=True) as cursor:
            cursor.execute("""SELECT u.id, u.name, u.email, u.region
                                FROM users AS u
